...
where 	pl.viewPeriodId in (select id from (
			(select cf.createAndJoinPeriodId as id from competitionConfigs cf join competitions c on c.id = cf.sourceCompetitionId  join leagues l on l.id = c.leagueId  join seasons s on c.seasonId = s.id where l.name = 'Eredvisie' and s.startDateTime <= '2015-07-01' and s.endDateTime >= '2014-07-01')
			union
			(select assPer.viewPeriodId as id from assemblePeriods assPer join competitionConfigs cf on cf.assemblePeriodId = assPer.id join competitions c on c.id = cf.sourceCompetitionId  join leagues l on l.id = c.leagueId  join seasons s on c.seasonId = s.id where l.name = 'Eredvisie' and s.startDateTime <= '2015-07-01' and s.endDateTime >= '2014-07-01')
			union
...
where 	l.name = 'Eredvisie' and s.startDateTime <= '2015-07-01' and s.endDateTime >= '2014-07-01';
...
from		competitions c
		join seasons s on c.seasonId = s.id
		join leagues l on l.id = c.leagueId  
where 	l.name in ('Competition','Cup', 'SuperCup') and s.startDateTime <= '2015-07-01' and s.endDateTime >= '2014-07-01';
...
from		teamPlayers tmPl 
		...
		join leagues l on l.id = c.leagueId  
where 	l.name = 'Eredvisie' and s.startDateTime <= '2015-07-01' and s.endDateTime >= '2014-07-01'
and	 	tmPl.startDateTime <= '2015-07-01' and tmPl.endDateTime >= '2014-07-01';